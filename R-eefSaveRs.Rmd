...
lat_max <- -10   # Northernmost latitude
...
# Define the bounding box for the Great Barrier Reef area
bounding_box <- c(left = 142, bottom = -24, right = 154, top = -10)

# Get the map
reef_map <- get_stamenmap(bbox = bounding_box, zoom = 6, maptype = "terrain")

ggmap(reef_map) +
  geom_point(data = reef_data, aes(x = LONGITUDE, y = LATITUDE, color = reef_zone), alpha = 0.7, size = 2) +
  scale_color_manual(values = c("A" = "red", "B" = "blue", "C" = "green")) +
  ggtitle("Great Barrier Reef Zones") +
  theme_minimal()
