...
```{r querying GBIFs API, echo=FALSE, cache = TRUE, eval=FALSE, include=TRUE}
...
# Maps of Zones A, B, and C based on Severity (Green = Least Severe and Red = Most Severe)
...
ggplot(species_observations_per_zone_year, aes(x = year, y = observation_ratio, color = species)) +
...
## Time Series
```{r vis1, include=TRUE}
# Time-Series Analysis of Observation Count
ggplot(severity_code_by_year_zone, aes(x = YEAR, y = avg_severity)) +
  geom_line() +
  facet_wrap(~reef_zone) +
  labs(title = "Time-Series Analysis of Average Severity Code by Year and Reef Zone",
       x = "Year", y = "Average Severity Code") +
  theme_minimal()
...
ggplot(species_observations_per_zone_year, aes(x = year, y = species, fill = observation_ratio)) +
  geom_tile() +
...
ggplot(shannon_diversity, aes(x = year, y = diversity, color = reef_zone)) +
  geom_line() +
