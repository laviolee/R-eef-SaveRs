...
## Objectives
- Primary Objective: To analyze the relationship between environmental severity, as indicated by coral bleaching events, and the occurrence of key marine species in the Great Barrier Reef.
- Secondary Objective: To understand the spatial distribution of these impacts across different zones within the reef and over various time periods.
...
```{r vis5, include=FALSE}

p <- ggplot(species_observations_per_zone_year, aes(x = species, y = observation_ratio, fill = factor(year))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  scale_y_continuous(labels = scales::percent) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +
  labs(title = "Comparative Ratio of Observation Counts by Species and Year",
       x = "Species", y = "Observation Count Ratio")
```
...
In this analysis, PC1 and PC2 together form a plane where species are plotted, revealing clustering and potential outliers. PC1 appears to be influenced heavily by the maximum observation counts, suggesting that species with higher overall observations are spread along this axis. PC2 is most impacted by the observation ratio, indicating that this component captures the relative frequency of observations per species. The clusters observed may represent groups of species with similar observation profiles, possibly linked to their ecological characteristics or response to environmental pressures. Outliers could signify species with unique or extreme observation records. The dispersion of points across the two principal components underscores the multifaceted nature of species distribution and abundance, reflecting the complexity of ecological interactions within the coral reef ecosystem. This PCA thus provides a foundational understanding of the data structure, guiding further, more nuanced analyses.
